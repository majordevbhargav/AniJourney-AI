        ...
        <form onSubmit={recommend} className="sticker-card rounded-3xl p-6 mb-16" data-testid="recommend-form">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <input
              ...
              className="flex-1 bg-transparent border-b border-sky-200 focus:border-rose-500 outline-none py-3 text-slate-900 text-lg"
            />
            <button disabled={busy} className="btn-coral" data-testid="recommend-submit-btn">
          ...
          {recs.length > 0 && (
            <div className="mt-6 grid gap-3" data-testid="recommendation-results">
              {recs.map((r, i) => (
                <div key={i} className="border-l-2 border-rose-500 pl-4 py-2">
                  <div className="font-display text-xl">{r.title}</div>
                  <div className="text-sm text-slate-500 italic">{r.reason}</div>
                </div>
              ))}
            </div>
          )}
            ...
            <Link key={a.id} to={`/anime/${a.id}`} className="sticker-card rounded-3xl overflow-hidden block group" data-testid={`anime-card-${a.id}`}>
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  ...
                  <div className="font-display text-2xl leading-tight text-white">{a.title}</div>
                </div>
              </div>
              <div className="p-5">
